    ...
    public class TenantShellFactory : ITenantShellFactory<Tenant>
    {
        public Task<TenantShell<Tenant>> Get(TenantIdentifier distinguisher)
        {
            if (distinguisher.Uri.Port == 5004)
            {
                ...
                var tenant = new Tenant(tenantId) { Name = "Moogle" };
                var result = new TenantShell<Tenant>(tenant);
                return Task.FromResult(result);
            }

            if (distinguisher.Uri.Port == 5000 || distinguisher.Uri.Port == 5001 || distinguisher.Uri.Port == 44354)
            {
                Guid tenantId = Guid.Parse("049c8cc4-3660-41c7-92f0-85430452be22");
                var tenant = new Tenant(tenantId) { Name = "Gicrosoft" };
                var result = new TenantShell<Tenant>(tenant, new Uri("http://localhost:5000"), new Uri("http://localhost:5001"), new Uri("http://localhost:44354")); // additional distinguishers to map this same tenant shell instance too.
                return Task.FromResult(result);
            }
